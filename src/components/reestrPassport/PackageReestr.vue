                  ...
                  <Row :gutter="16">
                    <Col :xs="{span: 24}" :md="{span: 12}">
                      <FormItem label="Время создания записи" :style="{width: '100%'}">
                        <DatePicker size="large" type="datetime" format="yyyy-MM-dd HH:mm" v-model="filter.createdTime.value1"
                                    placeholder="Период от"></DatePicker>
                      </FormItem>
                    </Col>
                    <Col :xs="{span: 24}" :md="{span: 12}">
                      <FormItem label=" " :style="{width: '100%'}">
                        <DatePicker size="large" type="datetime" format="yyyy-MM-dd HH:mm" v-model="filter.createdTime.value2"
                                    placeholder="Период по"></DatePicker>
                      </FormItem>
                    </Col>
                  </Row>
      ...
      downloadBody(packageItem) {
        ...
        (async () => {
          try {
            ...
            element.setAttribute('download', packageItem.name + '.' + packageItem.formatName.toLowerCase());
      ...
      packages: function () {
        ...
        if (data) {
          data.data.forEach((item) => {
            item.createTime = funcUtils.parseDateTime(item.createTime, 'DD.MM.YYYY HH:mm');
            item.signingTime = funcUtils.parseDateTime(item.signingTime, 'DD.MM.YYYY HH:mm');
          });
          res = data.data;
        }
        return res;
